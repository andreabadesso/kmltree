...
"balloonclose",b);g();google.earth.addEventListener(i,"balloonopening",e);google.earth.addEventListener(i,"balloonclose",b)};var e=function(g){g=g.getFeature();m(g);console.log("balloonopening")},b=function(){},c=function(g,h){if(g.getUrl()===h)return tree;if(g.getElementByUrl(h))return tree},m=function(g){g=g.getUrl();var h=g.split("#")[0];if(k[h]){console.log("cache hit");return k[h]}else console.log("cache miss");for(var j=0;j<l.length;j++)if(c(l[j].instance.kmlObject,g)){k[h]=l[j];return l[j]}for(j=
0;j<l.length;j++)for(doc in l[j].api.docs)if(c(doc,g)){k[h]=l[j];return l[j]}return false};$(window).bind("message",function(g){g=g.originalEvent;var h=g;console.log("resize");g=i.getBalloon();var j=g.getFeature(),t;if(!(t=!$("#kmltree-balloon-iframe").length))if(!(t=!(h.data.match(/width/)||h.data.match(/unknownIframeDimensions/))))if(!(t=g.getType()!=="GEHtmlDivBalloon")){t=$(g.getContentDiv()).find("iframe")[0];var q;b:for(var s=h.source,z=document.getElementsByTagName("iframe"),C=0;C<z.length;C++)if(z[0].contentWindow===
s){q=z[C];break b}t=t!==q}if(!t){q=m(j);h=JSON.parse(h.data);if(h.unknownIframeDimensions){h=q.api.opts.unknownIframeDimensionsDefault;console.log(h)}t=$("#kmltree-balloon-iframe");g.setMinWidth(h.width);g.setMaxWidth(h.width+h.width*0.1);g.setMinHeight(h.height);g.setMaxHeight(h.height+h.height*0.1);t.height(h.height);t.width(h.width);console.log("set dimensions");$(q.instance).trigger("balloonopen",[g,j])}});return that}(),Base64={_keyStr:"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=",
...
var kmltree=function(){function l(b){var c=document.createElement("a");c.href=b;return c.href}openBalloon=function(b,c,m,g){var h;if(m.displayEnhancedContent){var j=b.getBalloonHtmlUnsafe();h=b.getBalloonHtml();h=$.trim(h.replace(/\s*<\!--\s*Content-type: mhtml-die-die-die\s*--\>/,""));h=h!=$.trim(j)}if(m.displayEnhancedContent&&h){h=c.createHtmlDivBalloon("");var t=$(['<iframe id="kmltree-balloon-iframe" border="0" frameBorder="0" src="',m.iframeSandbox,'"></iframe>'].join("")),q=document.createElement("div");
$(q).append(t);t.load(function(){this.contentWindow.postMessage(JSON.stringify({html:Base64.encode(j),callback:Base64.encode(m.sandboxedBalloonCallback.toString())}),"*")});h.setContentDiv(q)}else{h=c.createFeatureBalloon("");var s=function(z){setTimeout(function(){$(g).trigger("balloonopen",[z.getBalloon(),z.getFeature()])},1);google.earth.removeEventListener(c,"balloonopening",s)};google.earth.addEventListener(c,"balloonopening",s)}h.setFeature(b);c.setBalloon(h);setTimeout(function(){window.kmltreeAlreadyGotItMkay=
...
e,b);var j=b.gex.pluginInstance,t=false,q={};parseFloat(j.getPluginVersion())<5.2&&alert("kmltree requires a google earth plugin version >= 5.2");if(!b.url||!b.gex||!b.element||!b.mapElement)throw"kmltree requires options url, gex, mapElement & element";b.element=$(b.element);if(!b.element.attr("id")){b.element.attr("id","kml-tree"+(new Date).getTime());b.element.attr("UNSELECTABLE","on")}b.restoreState&&$(window).unload(function(){c.destroy()});b.element.css("position")!=="absolute"&&$(b.element).css({position:"relative"});
var s=$('<div class="kmltree" style="background-size: 16px 16px; -moz-background-size: 16px 16px; -o-background-size: 16px 16px; -webkit-background-size: 16px 16px; -khtml-background-size: 16px 16px;"></div>'),z=s[0].style.backgroundSize!==undefined||s[0].style.MozBackgroundSize!==undefined||s[0].style.oBackgroundSize!==undefined||s[0].style.khtmlBackgroundSize!==undefined||s[0].style.webkitBackgroundSize!==undefined,C=function(a,d,f){var n={name:a.getName(),id:"kml"+(f?f.replace(/\W/g,"-"):"")+d.replace(/\W/g,
"-")};google.earth.executeBatch(j,function(){b.gex.dom.walk({visitCallback:function(o){var r=o.current;if(!r.children)r.children=[];var u=this.getName(),p;p=r.id;key=u?u.replace(/\W/g,"-"):"blank";p+=key;for(var v=0;g[p];){v++;p+=v}g[p]=this;var H=this.getSnippet();if(!H||H==="empty")H=false;v=this.getType();var S=false;if(v==="KmlPlacemark"){var I=this.getGeometry();if(I)S=I.getType()}var E=this.getComputedStyle();u=u||"&nbsp;";I=!!this.getVisibility();var ca=this.getOpen(),da=this.getDescription(),
x="check";if(E=E.getListStyle())switch(E.getListItemType()){case 5:x="radioFolder";break;case 2:x="checkOffOnly";break;case 3:x="checkHideChildren"}E=x;x=false;if(z&&this.getType()==="KmlPlacemark"&&this.getGeometry()&&this.getGeometry().getType()==="KmlPoint")x=this.getComputedStyle().getIconStyle().getIcon().getHref();if(b.supportItemIcon)x=(x=kmldom(this.getKml()).find("kml>Folder, kml>Document, kml>Placemark, kml>NetworkLink").find(">Style>ListStyle>ItemIcon>href").text())?x:false;p={renderOptions:J,
name:u,visible:I,type:v,open:ca,id:p,description:da,snippet:H,select:false,listItemType:E,customIcon:x,customClass:"",children:[],alwaysRenderNodes:false,kmlId:this.getId().replace(/\W/g,"-"),geoType:S};p=b.visitFunction(this,p);r.children.push(p);if(p.listItemType!=="checkHideChildren")o.child=p;else o.walkChildren=false},rootObject:a,rootContext:n})});return n},V=function(a){if(c.kmlObject)throw"KML already loaded";T();var d=l(b.url);if(a||b.bustCache){a=(new Date).getTime();d=d.indexOf("?")===
-1?d+"?cachebuster="+a:d+"&cachebuster="+a}google.earth.fetchKml(j,d,function(f){t||U(f,d,b.url)})};c.load=V;c.refresh=function(){if(b.refreshWithState)c.previousState=M();W();g=null;g={};j.setBalloon(null);V(true)};var N=function(a){return b.element.find("."+a.replace(/\W/g,"-"))};c.getNodesById=N;c.selectById=function(a){a=N(a)[0];return O(a,w(a))};var B=function(a,d){var f=$("#"+b.element.attr("id")).find(".selected").removeClass("selected");if(f.length){f.each(function(){y($(this),"selected",
false)});d||$(c).trigger("select",[null,null])}j.getBalloon()&&!a&&j.setBalloon(null)};c.clearSelection=function(){return B()};var T=function(a){K();a=a||b.loadingMsg;a=$('<div class="kmltree-loading"><span>'+a+"</span></div>");var d=b.element.height();d!==0&&a.height(d);b.element.append(a)};c.showLoading=T;var K=function(){b.element.find(".kmltree-loading").remove()};c.hideLoading=K;var U=function(a,d,f){q={};if(a){m=0;c.kmlObject=a;h.push(a);c.kmlObject.setVisibility(true);var n=C(a,f),o;if(b.displayDocumentRoot===
...
0){A(f,false);Q(f,false)}},F=function(a,d){if(!(a.hasClass("checkOffOnly")&&d)){var f=a.parent().parent();f.hasClass("radioFolder")&&f.find(">ul>li.visible").each(function(){A($(this),false);G($(this),false)});A(a,d);d&&a.find("li.visible").length||G(a,d);Q(a,d)}},A=function(a,d){a=$(a);if(a.hasClass("visible")!==d){y(a,"visible",d);w(a).setVisibility(d);a.toggleClass("visible",d)}},y=function(a,d,f){a=a.attr("id");q[a]||(q[a]={});a=q[a];if(a[d])a[d].value=f;else a[d]={original:!f,value:f}},M=function(){for(var a in q){for(var d in q[a])q[a][d].original===
q[a][d].value&&delete q[a][d];var f=0;for(d in q[a])q[a].hasOwnProperty(d)&&f++;f===0&&delete q[a]}return q};c.getState=M;c.openNetworkLink=function(a){if(a.find("> ul").length)throw"networklink already loaded";else{var d=w(a),f=d.getLink();if(f)var n=f=f.getHref();else{var o=kmldom(d.getKml()).find("Url>href");if(o.length)n=f=o.text();else{a.addClass("error");$(a).trigger("loaded",[a,false]);a.removeClass("open");y(a,"open",false);return}}o=new URI(f);if(o.getAuthority()===null){var r=d.getOwnerDocument();
if(r&&r.getUrl)if(r=r.getUrl()){r=new URI(r);var u=o.resolve(r)}if(u)f=u.toString();else alert(["Could not resolve relative link in kml ","document. You may need to upgrade to the ","latest Google Earth Plugin."].join())}if(b.bustCache){u=(new Date).getTime();f=f.indexOf("?")===-1?f+"?cachebuster="+u:f+"&cachebuster="+u}a.addClass("loading");google.earth.fetchKml(j,f,function(p){if(p){j.getFeatures().appendChild(p);p.setVisibility(d.getVisibility());var v=d.getName();d.getParentNode().getFeatures().removeChild(d);
v=C(p,n,v);v=J(v.children[0].children);a.append("<ul>"+v+"</ul>");a.addClass("open");y(a,"open",a.hasClass("open"));a.removeClass("loading");a.addClass("loaded");g[a.attr("id")]=p;h.push(p);$(a).attr("data-rememberedLink",L.length.toString());L.push(d);$(a).trigger("loaded",[a,p]);$(c).trigger("networklinkload",[a,p])}else{alert("Error loading "+f);a.addClass("error");$(c).trigger("kmlLoadError",[p]);a.removeClass("open")}})}};var L=[];c.getNetworkLink=function(a){return(a=$(a).attr("data-rememberedLink"))&&
L.length>=a?L[a]:false};var aa=function(a){var d=new i({success:function(){},error:function(){},tree:c});d.add(a,function(f){f.addClass("open");y(f,"open",a.hasClass("open"));P(d,f)});d.execute()};s=b.element.attr("id");$("#"+s+" li > span.name").live("click",function(a){a.stopPropagation();a=$(this).parent();var d=w(a);if(a.hasClass("error")&&a.hasClass("KmlNetworkLink"))d.getLink()&&d.getLink().getHref()?alert("Could not load NetworkLink with url "+d.getLink().getHref()):alert("Could not load NetworkLink. Link tag with href not found");
if(a.hasClass("select"))O(a,d);else{B();if(a.hasClass("hasDescription")||d.getType()==="KmlPlacemark"){d.getType()==="KmlPlacemark"&&F(a,true);google.earth.removeEventListener(j,"balloonopening",D);openBalloon(d,j,b,c);google.earth.addEventListener(j,"balloonopening",D)}}$(c).trigger("click",[a[0],d])});$("#"+s+" li > span.name").live("contextmenu",function(){var a=$(this).parent(),d=w(a);$(c).trigger("contextmenu",[a[0],d])});b.element.click(function(a){if(a.target===this)B();else{a=$(a.target);
a.hasClass("toggle")&&!a.hasClass("select")&&B()}});$("#"+s+" li > .expander").live("click",function(){var a=$(this).parent();if(a.hasClass("KmlNetworkLink")&&!a.hasClass("loaded")&&!a.hasClass("loading"))aa(a);else{a.toggleClass("open");y(a,"open",a.hasClass("open"))}});$("#"+s+" li > .toggler").live("click",function(){var a=$(this).parent(),d=!a.hasClass("visible");!d&&a.hasClass("selected")&&B();!d&&j.getBalloon()&&j.setBalloon(null);if(!(a.hasClass("checkOffOnly")&&d)){if(a.hasClass("KmlNetworkLink")&&
a.hasClass("alwaysRenderNodes")&&!a.hasClass("open")&&!a.hasClass("loading")&&!a.hasClass("loaded")){aa(a);$(a).bind("loaded",function(f,n){F(n,true);n.removeClass("open")})}else F(a,d);$(c).trigger("toggleItem",[a,d,w(a)])}});$("#"+s+" li").live("dblclick",function(a){a.stopPropagation();var d=$(a.target);a=d.parent();if(!(d.hasClass("expander")||d.hasClass("toggler")||a.hasClass("expander")||a.hasClass("toggler"))){d=$(this);var f=w(d);if(d.hasClass("error"))f.getLink()&&f.getLink().getHref()?alert("Could not load NetworkLink with url "+
f.getLink().getHref()):alert("Could not load NetworkLink. Link tag with href not found");else{F(d,true);if(f.getType()=="KmlTour")j.getTourPlayer().setTour(f);else{var n=null,o=$(b.mapElement);if(o.length)n=o.width()/o.height();b.gex.util.flyToObject(f,{boundsFallback:a.find("li").length<1E3,aspectRatio:n})}$(c).trigger("dblclick",[d,f])}}});s=google.earth.addEventListener;var D=function(a){if(window.kmltreeAlreadyGotItMkay)console.log("somebody already has it",window.kmltreeAlreadyGotItMkay);else{var d=
a.getFeature();if(ba(d)){window.kmltreeAlreadyGotItMkay=c;a.preventDefault();a.stopPropagation();j.setBalloon(null);openBalloon(d,j,b,c);return false}}};google.earth.addEventListener(j,"balloonopening",D);var Z=function(){$("#kmltree-balloon-iframe").remove();B()};s(j,"balloonclose",Z);var R=false;s(j.getGlobe(),"dblclick",function(a){if(a.getButton()!==-1){var d=a.getTarget();if(d.getType()!=="GEGlobe")if(d.getType()==="KmlPlacemark"){a=d.getId();var f=N(a);if(f.length>=1)if(!R){R=true;setTimeout(function(){R=
